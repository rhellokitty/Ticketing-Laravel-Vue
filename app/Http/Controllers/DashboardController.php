...
class DashboardController extends Controller
{
    public function getStatistics()
    {
        ...
        $resolvedTickets = Ticket::whereBetween('created_at', [$currentMonth, $endMonth])
            ->where('status', 'resolved')
            ->count();

        $avgResolutionTime = Ticket::where('created_at', [$currentMonth, $endMonth])
            ->where('status', 'resolved')
        ...
        $dashboardData = [
            ...
            'status_distribution' => $satusDistribution,
        ];
